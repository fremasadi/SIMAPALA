...
@section('content')
    ...
    <section class="py-28 bg-gradient-to-br from-gray-50 to-amber-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if (session('success'))
                <div
                    class="mb-6 bg-green-50 border-2 border-green-200 text-green-700 px-6 py-4 rounded-xl flex items-center">
            ...
            <div class="bg-white rounded-xl p-6 mb-6 border-2 border-gray-200 shadow-md">
                <form method="GET" action="{{ route('transactions.index') }}" class="grid md:grid-cols-4 gap-4">
                    ...
                    <div class="flex items-end">
                        <button type="submit"
                            class="w-full px-6 py-2.5 bg-[#1F2937] text-white rounded-lg hover:from-amber-600 hover:to-orange-600 transition font-bold shadow-md">
            ...
            @if ($transactions->isEmpty())
                <div class="bg-white rounded-2xl p-12 text-center border-2 border-gray-200">
                    ...
                    <a href="/#equipment"
                        class="inline-block px-8 py-3 bg-[#EAB308] text-white rounded-lg hover:bg-[#1F2937] transition font-semibold shadow-md">
                        Mulai Sewa Alat
                    </a>
                </div>
            @else
                <div class="space-y-4">
                    @foreach ($transactions as $transaction)
                        <div
                            class="bg-white rounded-xl shadow-md border-2 border-gray-100 hover:border-amber-300 transition overflow-hidden">
                            <div class="p-6">
                                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                                    ...
                                    <div class="flex-1 mb-4 lg:mb-0">
                                        ...
                                        <div class="grid md:grid-cols-3 gap-3 mb-3">
                                            <div class="flex items-center text-sm">
                                                ...
                                                <span
                                                    class="text-gray-600">{{ $transaction->tanggal_pinjam->format('d M Y') }}</span>
                                            </div>
                                            <div class="flex items-center text-sm">
                                                <svg class="w-4 h-4 text-gray-400 mr-2" fill="none"
                                                    stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                <span
                                                    class="text-gray-600">{{ $transaction->tanggal_pinjam->diffInDays($transaction->tanggal_kembali) }}
                                                    hari</span>
                                            </div>
                                            <div class="flex items-center text-sm">
                                                <svg class="w-4 h-4 text-gray-400 mr-2" fill="none"
                                                ...
                                                <span class="text-gray-600">{{ $transaction->detailTransaksis->count() }}
                                                    alat</span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex flex-wrap gap-2 mb-3">
                                            @foreach ($transaction->detailTransaksis->take(3) as $detail)
                                                <span
                                                    class="inline-flex items-center px-3 py-1 bg-amber-50 text-amber-700 rounded-full text-xs font-semibold border border-amber-200">
                                                    {{ $detail->alat->nama_alat }}
                                                </span>
                                            @endforeach
                                            @if ($transaction->detailTransaksis->count() > 3)
                                                <span
                                                    class="inline-flex items-center px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-xs font-semibold">
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            @endphp
                                            <span
                                                class="inline-flex items-center px-3 py-1 rounded-lg text-sm font-bold border-2 {{ $status['class'] }}">
                                                {{ $status['label'] }}
                                            </span>

                                            @if ($transaction->pembayaran && $transaction->pembayaran->isSuccess())
                                                <span
                                                    class="ml-2 inline-flex items-center px-3 py-1 rounded-lg text-xs font-bold bg-green-100 text-green-800 border border-green-300">
                                    ...
                                    {{-- Right: Price & Actions --}}
                                    <div
                                        class="lg:text-right lg:ml-6 border-t lg:border-t-0 lg:border-l-2 border-gray-100 pt-4 lg:pt-0 lg:pl-6">
